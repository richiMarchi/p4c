...
#include "backends/polycube/version.h"
#include "ir/ir.h"
#include "lib/log.h"
#include "lib/crash.h"
#include "lib/gc.h"
...
void compile(PolycubeOptions& options) {
    ...
    POLYCUBE::run_polycube_backend(options, toplevel, &midend.refMap, &midend.typeMap);
}
...
int main(int argc, char *const argv[]) {
    ...
    AutoCompileContext autoPolycubeContext(new PolycubeContext);
    auto& options = PolycubeContext::get().options();
    options.compilerVersion = P4C_POLYCUBE_VERSION_STRING;
    ...
    if (::errorCount() == 0)
        std::cout << "****** .:DATA PLANE SUCCESSFULLY TRANSLATED:. ******" << std::endl;
    else
        std::cout << "****** .:ERRORS DURING DATA PLANE TRANSLATION:. ******" << std::endl;
    return ::errorCount() > 0;
}
