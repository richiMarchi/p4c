...
bool CodeGenInspector::preorder(const IR::Declaration_Variable* decl) {
    auto type = PolycubeTypeFactory::instance->create(decl->type);
...
bool CodeGenInspector::preorder(const IR::Cast* c) {
    ...
    builder->append("(");
    builder->append("(");
    auto et = PolycubeTypeFactory::instance->create(c->destType);
    et->emit(builder);
    builder->append(")");
    visit(c->expr);
    builder->append(")");
...
bool CodeGenInspector::preorder(const IR::Member* expression) {
    ...
    if (ei == nullptr) {
        ...
        auto p4type = expression->expr->type->getP4Type();
        //TODO: try to make it more dynamic instead of using string checks
        if ((p4type && (p4type->toString() == PolycubeControl::metadataTypeName))
                || expression->expr->type->is<IR::Type_Header>()) {
            builder->append("->");
        } else {
            builder->append(".");
        }
...
bool CodeGenInspector::preorder(const IR::Type_Typedef* type) {
    auto et = PolycubeTypeFactory::instance->create(type->type);
...
bool CodeGenInspector::preorder(const IR::AssignmentStatement* a) {
    ...
    auto polycubeType = PolycubeTypeFactory::instance->create(ltype);
    bool memcpy = false;
...
void CodeGenInspector::widthCheck(const IR::Node* node) const {
    ...
    if (tb->size % 8 == 0 && PolycubeScalarType::generatesScalar(tb->size))
        return;
